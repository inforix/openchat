...
- bot 列表从 OpenClaw 顶层 `bindings` 中的 canonical OpenChat 绑定投影得到
...
当前仓库仍然没有把 `apps/web -> apps/relay -> apps/edge` 全链路接成真实联网运行。现阶段的端到端演示仍然主要依赖浏览器侧 harness；虽然 `apps/edge` 已有真实入口，但 Web 侧还没有接入真实 relay/edge transport。
...
- 当前实现不再依赖 `channels.openchat.accounts`；bot 配置来源于 OpenClaw 顶层 `bindings` 中 canonical OpenChat 绑定的投影。
- 这条路径已能兼容本机 stock OpenClaw 的 `config get/set bindings` 能力，但更高层的 OpenClaw CLI 仍未原生暴露 `openchat` channel 管理命令。
- canonical OpenChat 绑定记录形状固定为 `{ "agentId": "...", "match": { "channel": "openchat", "accountId": "..." } }`。
- 写入 OpenChat bot 时会保留 `bindings` 中其他 channel 的记录；如果同一个 `accountId` 同时绑定到多个 `agentId`，OpenChat 会直接报错而不是猜测使用哪一个。
